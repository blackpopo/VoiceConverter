...
config = SPConfig()
import datetime
from yukarin_model import Generator
...
def train_step(input_image, target, epoch, generator, generator_optimizer, summary_writer):
    with tf.GradientTape() as gen_tape:
        ...
        gen_loss = tf.reduce_mean(tf.abs(target-gen_output)) * 100
...
def fit(dataset, epochs, test_dataset,  save_dir, summary_writer, mode, loading=False):
    ...
    if loading:
        generator = tf.keras.models.load_model(os.path.join(save_dir, 'become_yukarin1D_generator_epoch_' + str(90) + '_' + mode))
    elif mode == 'mel':
        generator = Generator()
    else:
...
def _batch_parser_sp(record_batch):
    ...
    src_sp = parsed["src_sp"]
    tgt_sp = parsed["dst_sp"]
...
def _batch_parser_f0(record_batch):
    ...
    tgt_who = parsed["dst_who"]
    src_f0 = parsed["src_f0"]
    tgt_f0 = parsed["dst_f0"]
...
def _batch_parser_mel(record_batch):
    ...
    tgt_who = parsed["dst_who"]
    src_mel = parsed["src_mel"]
    tgt_mel = parsed["dst_mel"]
...
def train():
    ...
    log_dir = "log"
    save_dir = '../ModelData2'
    summary_writer = tf.summary.create_file_writer(log_dir  + "/" + mode +'_' + 'become_yukarin' + '_B' + str(config.BATCH_SIZE) + '_'+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
